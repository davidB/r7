define ["r7/evt", "r7/Position", "r7/Rules4TargetG1"], (evt, Position, Rules4TargetG1) ->
  ...
  describe "GameRules:TargetG1", ->
    it "should spawn target once at first ship spawn", ->
      ...
      sut evt.SpawnObj(shipId, Position(3, 5, 0), null), out
      r = evt.extractK(out, "SpawnObj")
      expect(r.length).toEqual 1
      i = 0

      while i < 3
        sut evt.SpawnObj(shipId, Position(3 + 1, 5 - i, 0), null), out
        r = evt.extractK(out, "SpawnObj")
        expect(r.length).toEqual 1
        i++
    ...
    describe "on hit", ->
      ...
      beforeEach ->
        ...
        sut.onEvent evt.SpawnObj(shipId, Position(3, 5, 0), null), out
        r = evt.extractK(out, "SpawnObj")
        expect(r.length).toEqual 1
        tid = r[0].objId
        out.length = 0
